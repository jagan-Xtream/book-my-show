...
router.get("/:ticket_id", (req, res) => {
  Movie.findById(req.params.id, (err, foundMovie) => {
    if (err) {
      ...
      const foundTicket = ordersCopy.filter((ticket) =>
        ticket._id.equals(req.params.ticket_id)
      )[0];
...
router.delete("/:ticket_id", (req, res) => {
  Movie.findById(req.params.id, (err, foundMovie) => {
    if (err) {
      ...
      let idx = foundMovie.orders.findIndex((order) =>
        order._id.equals(req.params.ticket_id)
      );
      foundMovie.orders.splice(idx, 1);
      foundMovie.save();
