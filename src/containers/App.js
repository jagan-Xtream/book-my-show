...
class App extends Component {
      ...
      axios
        .put(
          "http://localhost:5000/movies/" +
            this.state.selectedMovie._id +
            "/tickets/" +
            this.state.editTicketId,
          ticketOrder
        )
        .then((res) => {
  ...
  deleteOrderHandler = (ticketId) => {
    ...
    const updatedOrders = this.state.selectedMovie.orders.filter(
    ...
    const selectedMovieIdx = this.state.movies.indexOf(
      this.state.selectedMovie
    );
    const updatedMovies = [...this.state.movies];
  ...
  render() {
    return (
      <div className={classes.App}>
        ...
        <p className={classes.AboutBtn} onClick={this.toggleAbout}>
          About
        </p>
        <About
